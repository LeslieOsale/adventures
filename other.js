        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const EXCHANGE_RATE = 130; // 130 KES = 1 USD
            ...
            function convertCurrency(amount, fromCurrency, toCurrency) {
                ...
                // Always convert from KES to USD
                ...
                console.error('Unsupported currency conversion. Only KES to USD conversion is supported.', fromCurrency, toCurrency);
                return amount;
            }
            ...
            function recalcAmount() {
                ...
                const selectedCurrency = modalBookingForm.currency.value;
                console.log('Selected currency (base):', selectedCurrency);

                // Improved price parsing - now KES is primary
                let kesPrice = parseFloat(priceStr.match(/KES (\d+(,\d{3})*(\.\d+)?)/)?.[1]?.replace(/,/g, '') || 0);
                let usdPrice = parseFloat(priceStr.match(/USD \$?(\d+(\.\d+)?)/)?.[1] || 0);

                console.log('Parsed KES price (primary):', kesPrice);
                console.log('Parsed USD price (secondary):', usdPrice);
                ...
                // Ensure no concatenation of prices - prefer KES as base
                if (usdPrice > 0 && kesPrice > 0) {
                    console.log('Both KES and USD prices are valid. KES is base currency.');
                } else if (kesPrice > 0 && !usdPrice) {
                    // If we only have KES, convert to USD
                    usdPrice = convertCurrency(kesPrice, 'KES', 'USD');
                    console.log('Only KES found, converted to USD:', usdPrice);
                } else if (usdPrice > 0 && !kesPrice) {
                    // If we only have USD, this shouldn't happen with new format, but handle it
                    kesPrice = usdPrice * EXCHANGE_RATE;
                    console.log('Only USD found, calculated KES:', kesPrice);
                }
                ...
                const basePrice = selectedCurrency === 'KES' ? kesPrice : usdPrice;
                console.log('Base price in selected currency (' + selectedCurrency + '):', basePrice);
                ...
                if (modalBookingForm.paymentType.value === 'full') {
                    ...
                    modalAmount.readOnly = true;
                    console.log('Full payment amount set to:', modalAmount.value);
                } else {
                    modalAmount.value = '';
                    modalAmount.readOnly = false;
                    console.log('Partial payment mode - amount is editable');
                }
            ...
            modalBookingForm.currency.forEach(radio => {
                radio.addEventListener('change', () => {
                    ...
                    // Extract prices from the string - KES is primary
                    let kesPrice = parseFloat(priceStr.match(/KES (\d+(,\d{3})*(\.\d+)?)/)?.[1]?.replace(/,/g, '') || 0);
                    let usdPrice = parseFloat(priceStr.match(/USD \$?(\d+(\.\d+)?)/)?.[1] || 0);
                    ...
                    console.log('Base price for', selectedCurrency, ':', basePrice);
            ...
            function updateConfirmDetails() {
                ...
                // Extract price information correctly - KES is primary
                let priceStr = currentTrip.price || '';
                let origCurrency = priceStr.includes('KES') ? 'KES' : 'USD';
                let priceNum = 0;
                if (origCurrency === 'KES') {
                ...
                console.log('updateConfirmDetails - Original currency:', origCurrency);
                console.log('updateConfirmDetails - Price number:', priceNum);
                ...
                if (origCurrency === 'KES') {
                    // KES is primary, convert to USD if needed
                    convertedAmount = convertCurrency(priceNum, 'KES', 'USD');
                    kesTotal = (priceNum * adults) + (priceNum * 0.7 * children);
                    usdTotal = (convertedAmount * adults) + (convertedAmount * 0.7 * children);
                } else {
                    // USD is primary, convert to KES
                    convertedAmount = priceNum * EXCHANGE_RATE;
                    usdTotal = (priceNum * adults) + (priceNum * 0.7 * children);
                    kesTotal = (convertedAmount * adults) + (convertedAmount * 0.7 * children);
                }
                
                console.log('updateConfirmDetails - KES Total:', kesTotal);
                console.log('updateConfirmDetails - USD Total:', usdTotal);
                
                // Format the display amounts - KES primary, USD secondary
                let displayAmount;
                if (modalBookingForm.paymentType.value === 'full') {
...
Phone: ${modalPhone.value}${participantNames ? '\n\nAdditional Participants:\n' + participantNames : ''}`;
            ...
            modalBookingForm.addEventListener('submit', function(e) {
                ...
                const bookingRecord = {
                    ...
                    phone: phone,
                    referralCode: document.getElementById('modalReferralCode').value.trim() || null,
                    // Add explicit reference field (prefer referral code if provided)
                    reference: (document.getElementById('modalReferralCode').value.trim() || null),
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (bookingForm) {
        bookingForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                showPaymentMessage('Payment request sent. Checking transaction status...', true);

                const pollTransactionStatus = async (id, timeoutMs = 2 * 60 * 1000, intervalMs = 2000) => {
                    const start = Date.now();
                    let attempts = 0;
                    const maxAttempts = 60; // Increased max attempts to allow for backoff
                    
                    // Optimized polling configuration
                    const pollConfig = {
                        initialDelay: 300,      // Start with 300ms (immediate response)
                        maxDelay: 2000,         // Cap at 2 seconds
                        backoffMultiplier: 1.2, // Gentle exponential backoff
                        maxConsecutiveErrors: 3 // Fail fast on persistent errors
                    };
                    
                    let consecutiveErrors = 0;
                    
                    console.group('🔄 POLLING TRANSACTION STATUS');
                    ...
                    while (Date.now() - start < timeoutMs && attempts < maxAttempts) {
                        try {
                            ...
                            // Handle both 200 OK and 404 Not Found responses
                            ...
                            if (!j) {
                                console.warn(`⚠️ Attempt ${attempts + 1}/${maxAttempts}: HTTP ${r.status} - No valid response`);
                                attempts++;
                                lastResponse = { httpStatus: r.status, body: null };
                                // Calculate exponential backoff: start at 300ms, increase gradually
                                const waitTime = Math.min(pollConfig.initialDelay * Math.pow(pollConfig.backoffMultiplier, attempts), pollConfig.maxDelay);
                            ...
                            lastResponse = j;
                            console.log(`📊 Attempt ${attempts + 1}/${maxAttempts}: Status = ${status} | Elapsed: ${elapsedSeconds}s`);
                            
                            // Log only key details to avoid console spam
                            if (status !== 'pending' && status !== 'unknown') {
                                console.log('Response:', j);
                            }
                            
                            // Check for definitive status - exit immediately on success/failure
                            if (status === 'success' || status === 'failed' || status === 'cancelled') {
                                console.log(`✅ Transaction reached definitive status ('${status}') after ${attempts + 1} attempts and ${elapsedSeconds} seconds`);
                                console.groupEnd();
                                return j;
                            }
                            
                            // Still pending or unknown - continue polling with exponential backoff
                            if (status === 'pending' || status === 'unknown') {
                                const nextWaitTime = Math.min(pollConfig.initialDelay * Math.pow(pollConfig.backoffMultiplier, attempts), pollConfig.maxDelay);
                                console.log(`⏳ Still ${status}... retrying in ${Math.round(nextWaitTime)}ms`);
                            }
                        } catch (err) {
                            consecutiveErrors++;
                            const elapsedSeconds = Math.round((Date.now() - start) / 1000);
                            console.error(`❌ Attempt ${attempts + 1}/${maxAttempts}: ${err.name} - ${err.message}`);
                            
                            // Fail fast if we have too many consecutive errors
                            if (consecutiveErrors >= pollConfig.maxConsecutiveErrors) {
                                ...
                                console.groupEnd();
                                return { status: 'error', attempts, elapsedSeconds, error: err.message };
                            }
                        }
                        
                        attempts++;
                        // Calculate exponential backoff: start at initialDelay and increase gently
                        const waitTime = Math.min(pollConfig.initialDelay * Math.pow(pollConfig.backoffMultiplier, Math.max(0, attempts - 1)), pollConfig.maxDelay);
                        await new Promise(res => setTimeout(res, waitTime));
                    }
                    
                    const totalElapsed = Math.round((Date.now() - start) / 1000);
                    console.error(`❌ PAYMENT NOT CONFIRMED — TIMED OUT after ${attempts} attempts and ${totalElapsed}s`);
                    if (lastResponse) console.error('🔎 Last server response:', lastResponse);
                    console.groupEnd();
                    return { status: 'timeout', attempts, elapsedSeconds: totalElapsed, lastResponse };
                };

                const result = await pollTransactionStatus(checkoutId, 2 * 60 * 1000, 2000);
                ...
                if (result.status === 'success') {
                    ...
                    console.group('💳 PAYMENT SUCCESS');
                    console.log('✅ Transaction Status: SUCCESS');
                    console.log('CheckoutRequestID:', checkoutId);
                    console.log('Booking ID:', lastBookingId);
                    console.log('Attempts taken:', result.attempts || 'Auto');
                    console.log('Time elapsed:', result.elapsedSeconds || 'Unknown', 'seconds');
                    console.log('Transaction Result:', result);
                    console.log('Amount:', bookingIndex >= 0 ? bookings[bookingIndex].amount : 'Unknown');
                    console.log('Customer Phone:', bookingIndex >= 0 ? bookings[bookingIndex].phone : 'Unknown');
                    console.groupEnd();
                    ...
                    // ❌ DEBUG: Client canceled or payment failed
                    console.group('🚫 PAYMENT CANCELLED/FAILED');
                    console.warn('❌ Transaction Status:', result.status.toUpperCase());
                    console.log('CheckoutRequestID:', checkoutId);
                    console.log('Booking ID:', lastBookingId);
                    console.log('Cancellation/Failure Reason:', result.resultDesc || 'User cancelled STK prompt');
                    console.log('Full Result:', result);
                    console.log('Booking Details:', bookingIndex >= 0 ? {
                        customerName: bookings[bookingIndex].customerName,
                        phone: bookings[bookingIndex].phone,
                        amount: bookings[bookingIndex].amount,
                        eventTitle: bookings[bookingIndex].eventTitle
                    } : 'Booking not found');
                    console.groupEnd();
                    ...
                    console.warn('🚨 POLLING TIMEOUT - Manual check needed');
                    console.log('Attempts made:', result.attempts, '| Time elapsed:', result.elapsedSeconds, 'seconds');
                    console.log('💡 SOLUTION: Attempting direct M-PESA query via backend...');
                    
                    // Attempt one final direct query to backend (which should now have callback from M-PESA by now)
                    try {
                        const finalCheck = await fetch('http://localhost:10000/transaction-status/' + encodeURIComponent(checkoutId));
                        if (finalCheck.ok) {
                            const finalData = await finalCheck.json();
                            console.log('🔍 Final direct check result:', finalData);
                            ...
                            if (finalData && finalData.status === 'success') {
                                ...
                                const booking = bookingIndex >= 0 ? bookings[bookingIndex] : null;
                                showFinalPaymentDialog('Payment successful. Your receipt will download automatically.', true, booking, finalData, checkoutId);
                                return;
                            }
                        }
                    } catch (e) {
                        console.warn('Final check failed:', e.message);
                    }
                    
                    showPaymentMessage('Payment confirmation timed out after ' + result.attempts + ' attempts. You can re-check status or wait a few moments.', false);
                    const confirmDiv = document.getElementById('modalConfirmDetails');
                    if (confirmDiv) {
                        ...
                        btn.onclick = async () => {
                            ...
                            const res = await pollTransactionStatus(lastId, 3 * 60 * 1000, 2000);
                            if (res && res.status === 'success') {
                                // ✅ DEBUG: Payment confirmed after manual check
                                console.group('💳 PAYMENT SUCCESS (MANUAL CHECK)');
                                console.log('✅ Transaction Status: SUCCESS');
                                console.log('CheckoutRequestID:', lastId);
                                ...
                                // ❌ DEBUG: Payment failed/cancelled after manual check
                                console.group('🚫 PAYMENT FAILED/CANCELLED (MANUAL CHECK)');
                                console.warn('❌ Transaction Status:', res.status.toUpperCase());
                                console.log('CheckoutRequestID:', lastId);
                                console.log('Reason:', res.resultDesc || 'Unknown');
                                ...
                                console.warn('🚨 Manual check also timed out after', res.attempts, 'attempts');
                                showPaymentMessage('Still no confirmation after ' + res.attempts + ' attempts. Please contact support or try again later.', false);
                                btn.disabled = false;
                                btn.textContent = 'Check payment status';
                            } else {
                                console.warn('❓ Unexpected status response:', res);
                                showPaymentMessage('Payment status unclear. Please check your phone or contact support.', false);
                                btn.disabled = false;
                                btn.textContent = 'Check payment status';
                            }
                        };
    ...
    function generateReceiptFromBooking(booking, transactionData) {
        try {
            // 🖨️ DEBUG: Receipt generation started
            console.group('🖨️ RECEIPT GENERATION');
            console.log('📄 Generating receipt for booking:', booking.id);
            console.log('Customer:', booking.customerName);
            console.log('Event:', booking.eventTitle);
            console.log('Amount:', booking.amount);
            console.log('Referral Code:', booking.referralCode || 'None');
            ...
            console.log('M-PESA Receipt Number:', mpesaReceipt || 'Not available');
            ...
            const referralCodeDisplay = (booking.referralCode && booking.referralCode.length > 0) ? booking.referralCode : '—';
                ...
                <div class="receipt-body">
                  ...
                  ${booking.referralCode ? `<div class="receipt-row"><div class="label">Referral Code:</div><div class="value">${referralCodeDisplay}</div></div>` : ''}
            ...
            Promise.all(loadPromises).then(() => {
                console.log('✅ Receipt images loaded, generating canvas...');
                // Render to canvas using html2canvas then save as A4 PDF via jsPDF with margins
                // Safety checks: ensure html2canvas and jsPDF are available
                if (typeof html2canvas === 'undefined') {
                    console.error('html2canvas is not available. Cannot generate PDF receipt.');
                    // fallback: open receipt HTML in new tab for manual print/save
                    const win = window.open('', '_blank');
                    if (win) {
                        win.document.write('<html><head><title>Receipt</title></head><body>' + receiptEl.innerHTML + '</body></html>');
                        win.document.close();
                    }
                    console.groupEnd();
                    // cleanup
                    const elCleanup = document.getElementById('starkville-temp-receipt');
                    if (elCleanup && elCleanup.parentNode) elCleanup.parentNode.removeChild(elCleanup);
                    return;
                }
                if (!window.jspdf || typeof window.jspdf.jsPDF === 'undefined') {
                    console.error('jsPDF is not available (window.jspdf). Cannot save PDF.');
                    const win2 = window.open('', '_blank');
                    if (win2) { win2.document.write('<html><head><title>Receipt</title></head><body>' + receiptEl.innerHTML + '</body></html>'); win2.document.close(); }
                    console.groupEnd();
                    const elCleanup2 = document.getElementById('starkville-temp-receipt');
                    if (elCleanup2 && elCleanup2.parentNode) elCleanup2.parentNode.removeChild(elCleanup2);
                    return;
                }
                const canvasScale = 2;
                ...
                try {
                    ...
                    // Prefer mpesa receipt number, otherwise include booking.reference (referral code) if present, else booking.id
                    const refPart = (mpesaReceipt && mpesaReceipt.length > 0) ? mpesaReceipt : (booking.reference && booking.reference.length > 0 ? booking.reference : (booking.id || 'receipt'));
                    // sanitize filename (remove spaces/slashes)
                    const safeRef = String(refPart).replace(/[^a-zA-Z0-9-_]/g, '-');
                    const fileName = `Starkville-Receipt-${safeRef}.pdf`;
                    console.log('📥 Saving PDF receipt as:', fileName);
                    ...
                    console.log('🧹 Receipt temporary element cleaned up');
                }
            }).catch(err => {
